  ...
  return (
    <div className="@container bg-[var(--bg)]">
      <div className="lg:overflow-y-auto lg:max-h-[calc(100vh-80px)]">
        ...
        <div className={`px-4 py-2 space-y-5 transition-[padding-left] duration-500 ${plExploring} ${
              ...
              <article key={i} className="group">
                ...
                <div className="flex items-center gap-2.5 mb-1.5">
                  ...
                  <div className="min-w-0">
                    ...
                    {breadcrumb && <div className="text-[12px] text-[var(--meta)] truncate">{domain} &rsaquo; {breadcrumb}</div>}
                  </div>
                </div>
                {/* Title */}
                ...
                {/* Snippet */}
                <p className="text-[13px] text-[var(--snippet)] leading-[1.58] line-clamp-2 sm:line-clamp-3 mt-0.5">{r.snippet}</p>
            ...
            <div className="mt-4 sm:mt-5 flex flex-wrap justify-center gap-2">
              {SUGGESTIONS.map((q, i) => (
                <button key={q} onClick={() => engine.handleSearch(q)}
                  className="text-xs sm:text-sm px-3 sm:px-4 py-1.5 sm:py-2 rounded-full bg-[var(--bg-elevated)] text-[var(--text-muted)] hover:text-[var(--accent)] hover:bg-[var(--chip-hover)] cursor-pointer transition-colors"
        ...
        <div className="sticky top-0 z-30 bg-[var(--bg)]/95 backdrop-blur-md pt-2 sm:pt-3 pb-2 sm:pb-3">
          <div className="flex items-center gap-3 px-3 sm:px-4 lg:pl-40 lg:pr-8 max-w-5xl">
            ...
            <form onSubmit={(e) => { e.preventDefault(); const q = new FormData(e.currentTarget).get("q") as string; if (q.trim()) engine.handleSearch(q.trim()); }}
              className="flex-1 flex items-center gap-2 bg-[var(--bg-card)] border border-[var(--border)] rounded-full shadow-sm hover:shadow-md transition-shadow px-3 sm:px-4"
            >
