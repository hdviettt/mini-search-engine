...
export default function SystemNode({ data }: { data: SystemNodeData }) {
  ...
  const pct = data.progress
    ? Math.round((data.progress.done / Math.max(data.progress.total, 1)) * 100)
    : 0;

  return (
    <div
      className="flex w-[170px] cursor-pointer bg-[var(--bg-card)] hover:bg-[var(--bg-elevated)] transition-colors"
