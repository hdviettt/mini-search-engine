...
export default function GroundedData({ activeStep, trace, overviewTrace }: GroundedDataProps) {
  if (!activeStep) {
    return (
      <div className="flex items-center justify-center h-full text-[10px] text-gray-700 px-3 text-center">
        Hover a pipeline step to see the underlying data it uses.
      </div>
    );
  }

  // Tokenize → show stopwords list + vocabulary sample
        ...
        {t.stopwords_removed.length > 0 && (
          <div className="text-[10px] text-gray-700">
            Removed: {t.stopwords_removed.map((w, i) => (
              <span key={i} className="line-through text-gray-600 mx-0.5">{w}</span>
            ))}
          </div>
        )}
      </div>
  ...
  // Index Lookup → show terms table from the inverted index
          ...
          {Object.entries(t.terms_found).map(([term, info]) => (
            <div key={term} className="p-1.5 bg-blue-500/5 border border-blue-500/20 rounded">
              ...
              <div className="flex items-center gap-2 mt-0.5">
                ...
                <span className="text-[9px] text-gray-600">IDF: {info.idf}</span>
              </div>
            </div>
          ))}
        </div>
        {t.terms_missing.length > 0 && (
          <div className="p-1.5 bg-yellow-500/5 border border-yellow-500/20 rounded text-[10px] text-yellow-500">
  ...
  // PageRank → show authority scores
  if (activeStep === "pagerank" && trace) {
    ...
    return (
      <div className="p-2 space-y-2">
        ...
        <div className="space-y-0.5">
          {t.top_scores.slice(0, 8).map((s, i) => (
            <div key={i} className="flex items-center gap-1.5 py-0.5 px-1 rounded hover:bg-indigo-500/5">
              <span className="text-[9px] text-gray-600 w-3">#{i + 1}</span>
              <div className="flex-1 min-w-0">
                <div className="text-[10px] text-gray-400 truncate">{s.title.replace(" - Wikipedia", "")}</div>
                <div className="h-1 bg-[#222] rounded-full overflow-hidden mt-0.5">
  ...
  // AI Fan-out → show expanded queries
  ...
  // AI Retrieval → show retrieved chunks
        ...
        <div className="space-y-1">
          {r.chunks.slice(0, 4).map((c, i) => (
            <div key={i} className="p-1.5 bg-purple-500/5 border border-purple-500/15 rounded">
              ...
              <div className="flex gap-2 mt-0.5 text-[8px] text-gray-700">
