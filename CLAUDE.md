...
5. cross-encoder reranks the top `RERANK_TOP_K` (40)
...
full viewport in CSS pixels and is then multiplied by the zoom. `body` was
`min-h-screen`, so on an 800px viewport it rendered 960px tall and every page at
desktop width carried a page scrollbar that scrolled nothing, sitting a few
pixels from the results list's own scrollbar. Measured: 160px of phantom
overflow at 1280x800, 300px at 1728x1000, 0 at 1100x800 where no zoom applies.
Use `var(--vph)`, which is the viewport height already divided by the zoom in
force. It has a pure-CSS fallback per breakpoint in `globals.css` and is
overridden before first paint from the measured `window.innerHeight` in
`layout.tsx`.

The Explore view has two scrollbars on purpose. It is a two pane split, pipeline
canvas left and results right, and each pane scrolls its own content.

**A streaming endpoint must not hold an open transaction.** `/api/overview/stream`
used `db_conn()`, so its pooled connection stayed checked out for the whole
Server-Sent Events response, which includes the LLM call and its retries, and
longer still if the client walks away before the generator is drained. Under the
default `autocommit = False` psycopg opens a transaction on the first statement
and holds it until the block exits, so the retrieval `SELECT` kept its relation
locks the entire time. One of these held a lock for sixteen minutes, the
`ALTER TABLE` in `init_db()` queued behind it, application startup never
finished, and two deploys failed their healthcheck. Note the shape: a
zero-downtime deploy cannot recover on its own, because the leaked connection
belongs to the old container and the old container is not retired until the new
one is healthy. Use `streaming_conn()`, which runs in autocommit.

**`init_db()` sets `lock_timeout` and `statement_timeout` on purpose.** It runs
during startup, and `ALTER TABLE` needs `ACCESS EXCLUSIVE`, so without a bound a
single stray session stops the app ever becoming ready. Both blocks already
tolerate not applying and the schema is unchanged on nearly every boot, so
failing fast is right. Do not remove them to "make the migration reliable".

**Groq retires models without notice, and the failure looks like an outage.**
`llama-3.3-70b-versatile` was decommissioned in September 2026. The API answered
`404 model_not_found` while the key stayed valid, and `/api/overview` kept
returning `200` with `overview: null`, so AI Overviews were dead for days
without a single failing request. `GROQ_MODEL` now defaults to
`openai/gpt-oss-120b`. When overviews stop, check the model list first:
`curl -H "Authorization: Bearer $GROQ_API_KEY" https://api.groq.com/openai/v1/models`.
Both no-overview paths now carry an `error` string rather than a bare null.
Keep it that way.

**Groq's `gpt-oss` models are reasoning models** and reasoning tokens come out of
`max_tokens`. `AI_OVERVIEW_MAX_TOKENS` is 300, which 120b handles, but
`gpt-oss-20b` spends the whole budget thinking and returns an empty string. Do
not set `reasoning_effort: low` either: it made the model emit fullwidth
`【1】` citation brackets, which the `[1]` parser on the frontend does not match.
